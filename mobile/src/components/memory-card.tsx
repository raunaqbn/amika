...
export const MemoryCard = memo(function MemoryCard({ memory, onReaction }: MemoryCardProps) {
  ...
  return <PaperCard style={styles.card}><Pressable accessibilityRole="button" accessibilityLabel={`Open memory by ${actor}`} onPress={openMemory}>
    ...
    {memory.imageUrl ? <Image source={imageSource(memory.imageUrl)} style={styles.image} contentFit="contain" cachePolicy="memory-disk" recyclingKey={memory.id} enforceEarlyResizing /> : <View style={styles.textOnly}><Text style={styles.bigQuote}>“</Text><Text style={styles.textOnlyCopy}>{memory.content}</Text></View>}
