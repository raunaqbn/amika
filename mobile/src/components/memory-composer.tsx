...
export function MemoryComposer({ initiallyOpen = false, compact = false, onSaved }: { initiallyOpen?: boolean; compact?: boolean; onSaved?: () => void }) {
  ...
  async function save() {
    if (!friendId) return Alert.alert('Choose a friend', 'Every memory belongs with someone. Add or choose a friend first.');
    ...
    try {
      ...
      await api('/api/memories', { method: 'POST', body: JSON.stringify({ friendId, content: content.trim(), imageUrl: uploaded?.url || null, visibility, sharedWithFriend: visibility === 'friends', memoryDate: new Date().toISOString() }) });
      await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      setContent(''); setImageUri(null); setOpen(false);
      onSaved?.();
...
const styles = StyleSheet.create({
  ...
  label: { fontFamily: type.heavy, color: colors.ink, fontSize: 12, textTransform: 'uppercase', letterSpacing: 1 }, chips: { gap: 8 }, chip: { backgroundColor: colors.white, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 22, ...border }, chipActive: { backgroundColor: colors.citrus }, chipText: { fontFamily: type.medium, color: colors.ink }, chipTextActive: { fontFamily: type.heavy }, help: { fontFamily: type.medium, color: colors.muted, lineHeight: 19 },
