...
export function MemoryComposer({ initiallyOpen = false, compact = false, initialFriendId, onSaved }: { initiallyOpen?: boolean; compact?: boolean; initialFriendId?: string; onSaved?: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      for (const [index, item] of media.entries()) {
        ...
        const stored = await uploadMemoryMedia({
          uri: item.uri,
          ownerId: user.id,
          ...
          onProgress: (percentage) => {
            ...
            setUploadStatus(`Uploading ${item.type} ${index + 1} of ${media.length} · ${percentage}%`);
          },
        });
        uploaded.push({
          type: item.type,
    ...
    finally { setSaving(false); setUploadStatus(null); }
  }
  ...
  return <View style={[styles.packet, compact && { marginHorizontal: 0 }]}>
    ...
    {open ? <View style={styles.body}>
      <View style={styles.photoRow}>
        <View style={styles.mediaPicker}>
          {media.length ? <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.previewList}>{media.map((item, index) => <View key={item.id} style={styles.previewWrap}>
            {item.type === 'image' ? <Image source={imageSource(item.uri)} style={styles.preview} contentFit="cover" alt={`Selected photo ${index + 1}`} /> : <View style={styles.videoPreview}><Video size={28} color={colors.white} /><Text style={styles.videoPreviewTitle}>Video ready</Text><Text numberOfLines={1} style={styles.videoPreviewMeta}>{item.fileName || 'Selected clip'}</Text></View>}
      ...
      <Button label={uploadStatus || 'Save today’s memory'} tone="ink" loading={saving} disabled={preparingPhoto} onPress={save} />
