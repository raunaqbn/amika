...
export function Screen({ title, eyebrow, children, scroll = true, right, ...props }: ScrollViewProps & { title: string; eyebrow?: string; children: React.ReactNode; scroll?: boolean; right?: React.ReactNode }) {
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}><View style={styles.header}><View style={{ flex: 1 }}>{eyebrow ? <Text style={styles.eyebrow}>{eyebrow}</Text> : null}<Text style={styles.title}>{title}</Text></View>{right || <Pressable accessibilityLabel="Open profile" onPress={() => router.push('/profile')}><Avatar name={user?.name || 'A'} uri={user?.profileImage} size={42} color={colors.citrus} /></Pressable>}</View>{scroll ? <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={styles.content} {...props}>{content}</ScrollView> : <View style={[styles.content, { flex: 1 }]}>{content}</View>}</SafeAreaView>;
}
