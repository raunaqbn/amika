...
export default function MessagesScreen() {
  const router = useRouter(); const [threads, setThreads] = useState<Thread[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  const load = useCallback(async () => { setLoading(true); setError(''); try { setThreads(await api<Thread[]>('/api/messages')); } catch (e) { setError(e instanceof Error ? e.message : 'Could not load messages.'); } finally { setLoading(false); } }, []);
  useFocusEffect(useCallback(() => { load(); }, [load]));
  return <Screen title="Messages" eyebrow="A quieter place to talk">{loading ? <ActivityIndicator color={colors.ink} style={{ marginVertical: 40 }} /> : error ? <ErrorState message={error} onRetry={load} /> : threads.length ? threads.map((thread, index) => <Pressable key={thread.id} onPress={() => router.push({ pathname: '/conversation/[id]', params: { id: thread.id, name: thread.name, image: thread.profileImage || '' } })}><PaperCard style={{ backgroundColor: index % 2 ? '#FFF2C9' : colors.white }}><View style={styles.row}><Avatar name={thread.name} uri={thread.profileImage} color={index % 2 ? colors.rose : colors.sky} /><View style={{ flex: 1 }}><View style={styles.nameRow}><Text style={styles.name}>{thread.name}</Text>{thread.unreadCount ? <View style={styles.badge}><Text style={styles.badgeText}>{thread.unreadCount}</Text></View> : null}</View><Text numberOfLines={1} style={styles.preview}>{thread.lastMessage || 'Say hello and make a new memory.'}</Text></View><ChevronRight size={19} color={colors.muted} /></View></PaperCard></Pressable>) : <EmptyState title="No conversations yet" body="Once you connect with an Amika friend, your private thread will be waiting here." />}</Screen>;
}
