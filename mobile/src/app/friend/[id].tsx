...
import { apiCached, getCachedApiData, imageSource } from '@/lib/api';
import { cacheMemoryDetails, getMemoryFeedSnapshot } from '@/lib/memory-feed';
...
function belongsToFriend(memory: Memory, friendId?: string) {
  return Boolean(friendId && (memory.friendId === friendId || memory.friend?.id === friendId));
}
...
export default function FriendProfileScreen() {
  ...
  const cachedMemories = getCachedApiData<Memory[]>(MEMORIES_PATH);
  const initialFriend = cachedFriends?.find((item) => item.id === id);
  const initialMemories = (cachedMemories || getMemoryFeedSnapshot().items).filter((memory) => belongsToFriend(memory, id));
  ...
  const [memoriesLoading, setMemoriesLoading] = useState(!cachedMemories);
  const [memoriesError, setMemoriesError] = useState('');

  useEffect(() => { cacheMemoryDetails(memories); }, [memories]);
  ...
  const loadMemories = useCallback(async () => {
    ...
    try {
      const items = await apiCached<Memory[]>(MEMORIES_PATH, { force: true });
      const friendshipMemories = items.filter((memory) => belongsToFriend(memory, id));
      cacheMemoryDetails(friendshipMemories);
      setMemories(friendshipMemories);
    } catch (loadError) {
      setMemoriesError(loadError instanceof Error ? loadError.message : 'Could not load this friendship’s memories.');
    } finally {
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
      ...
      {memoriesLoading && !memories.length ? <Spinner color={colors.ink} style={styles.memoryLoader} />
        : memoriesError && !memories.length ? <ErrorState message={memoriesError} onRetry={() => void loadMemories()} />
          : memories.length ? <View style={styles.memoryGrid}>{memories.map((memory) => <FriendMemoryTile key={memory.id} memory={memory} />)}</View>
            : <EmptyState title="Your shared history starts here" body="Keep one small moment from today and it will appear in this friendship." actionLabel="Add the first memory" onAction={() => router.push({ pathname: '/add', params: { friendId: friend.id } })} />}
...
const styles = StyleSheet.create({
  ...
  memoryLoader: { marginVertical: 28 },
  memoryGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, alignItems: 'flex-start' },
  ...
  memoryTileFooter: { minHeight: 29, justifyContent: 'center', paddingHorizontal: 8 },
  memoryTileDate: { fontFamily: type.heavy, color: colors.muted, fontSize: 9, textTransform: 'uppercase', letterSpacing: .6 },
});
