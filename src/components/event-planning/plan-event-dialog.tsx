  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Failed to create event plan');
      }

      const eventPlan = await response.json();
      onEventCreated?.();
      onOpenChange(false);

      // Navigate to the event planning page
      router.push(`/event-plans/${eventPlan.id}`);
    } catch (err: any) {
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              placeholder="e.g., Movie Night, Concert, Birthday Dinner"
              value={title}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              placeholder="Brief description of what you're planning..."
              value={description}
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Select friends to collaborate on this event
            </p>
